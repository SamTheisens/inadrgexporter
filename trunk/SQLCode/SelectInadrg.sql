...
(
SELECT 
{5} AS Kdrs, {6} AS Klsrs, CAST(REPLACE(p.KD_PASIEN, '-', '') AS INT) AS Norm, 3 AS Klsrawat, 0 AS Biaya,
row_number() OVER (ORDER BY ISNULL(k.TGL_KELUAR, k.TGL_MASUK) ASC) AS Recid, 
CASE WHEN un.KD_BAGIAN = 3 THEN 
	CASE WHEN DATEDIFF(Hour, k.TGL_MASUK + k.JAM_MASUK, k.TGL_KELUAR + k.JAM_KELUAR) > 6 THEN 1
	ELSE 2 END
ELSE un.KD_BAGIAN
END
AS Jnsrawat, 
k.TGL_MASUK AS Tglmsk, 
...
GETDATE()) >= 1 THEN DATEDIFF(year, p.TGL_LAHIR, k.TGL_MASUK) ELSE NULL END  AS UmurThn, 
...
NULL as TglDari, NULL as TglSampai, '{4}' AS NamaRs 
...
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 1 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS Dutama,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 2 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D1,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 3 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D2,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 4 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D3,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 5 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D4,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 6 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D5,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 7 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D6,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 8 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D7,
CASE WHEN (RANK() OVER(PARTITION BY md.KD_PASIEN,md.TGL_MASUK,md.KD_UNIT,md.URUT_MASUK ORDER BY up.PRIORITAS ASC)) = 9 THEN REPLACE(pn.KD_PENYAKIT, '.', '') END AS D8
...
(un.KD_BAGIAN = 3 AND DATEDIFF(Hour, k.TGL_MASUK + k.JAM_MASUK, k.TGL_KELUAR + k.JAM_KELUAR) > 6 AND k.TGL_KELUAR BETWEEN '{0}' AND '{1}') OR
(un.KD_BAGIAN = 3 AND k.TGL_MASUK BETWEEN '{0}' AND '{1}')
)
GROUP BY p.KD_PASIEN,k.TGL_MASUK, k.JAM_MASUK, k.TGL_KELUAR, k.JAM_KELUAR, k.CARA_KELUAR, p.TGL_LAHIR, p.JENIS_KELAMIN, un.KD_BAGIAN, per.BERAT_LAHIR
)
{3}